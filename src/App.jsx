  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        title: "公开内容边界",
        cards: [
          ["内部策略不公开", "非公开资料、账号细节、私密对话和商业判断不写进公开文档。"],
    ...
    sections: [
      ...
      },
      {
        ...
        cards: [
          ...
          ["能不能直接搬运内容？", "不要。公开内容要用自己的项目、自己的经验和自己的表达。"],
...
const tutorialScenarioSections = {
  "1-codex-intro": [
    {
      title: "实战情景：第一次把项目交给 Codex",
      list: [
        "选择一个没有生产密钥的小项目。",
        "要求 Codex 先只读扫描，不要修改文件。",
        "让它给出技术栈、入口文件、启动命令、测试命令和风险点。",
        "只接受能被 README、配置文件或脚本验证的结论。"
      ]
    },
    {
      title: "可复制提示词",
      code:
        "请先只读检查这个项目，不要修改文件。\n请输出：项目用途、技术栈、入口文件、核心目录、启动命令、测试命令、风险边界，以及 3 个适合新手练习的小任务。\n如果信息不足，请说明依据不足，不要编造。"
    },
    {
      title: "你应该拿到什么",
      cards: [
        ["项目地图", "知道项目是什么、从哪里启动、哪些目录最重要。"],
        ["风险边界", "知道哪些文件不能公开、不能提交、不能让 AI 直接碰。"],
        ["下一步任务", "拿到 3 个低风险练习任务，而不是一上来大改项目。"]
      ]
    }
  ],
  "2-desktop-setup": [
    {
      title: "实战情景：用桌面端跑通第一条闭环",
      list: [
        "打开本地仓库，让 Codex 识别 README、源码目录和启动命令。",
        "创建一个小任务，例如改一处文案、修一处移动端间距、补一条 README 入口。",
        "要求它完成后给出 diff、验证命令和剩余风险。",
        "如果是网页任务，至少看一次移动端和桌面端。"
      ]
    },
    {
      title: "桌面端任务提示词",
      code:
        "请在桌面端工作区完成一个小任务：\n目标：[写用户能看到的变化]\n范围：[允许修改的文件或目录]\n限制：不改密钥、部署配置和无关页面；移动端不能溢出。\n请先定位文件和最小方案，等我确认后再修改。"
    },
    {
      title: "验收清单",
      cards: [
        ["能看见", "功能或内容真的出现在页面里，不只是代码改了。"],
        ["能验证", "有构建、测试、截图或日志作为结果证据。"],
        ["能回滚", "改动范围小，出现问题时知道撤哪几个文件。"]
      ]
    }
  ],
  "3-agents-context": [
    {
      title: "实战情景：给项目写第一份 AGENTS.md",
      list: [
        "先让 Codex 只读扫描项目，整理命令和边界。",
        "把启动、构建、测试、预览、禁止目录写进 AGENTS.md。",
        "把公开内容边界写清楚，避免把内部对话或私密研究写进页面。",
        "把移动端验收、敏感信息扫描和人工确认点写成固定规则。"
      ]
    },
    {
      title: "AGENTS.md 生成提示词",
      code:
        "请根据当前仓库生成一份 AGENTS.md 草案。\n必须包含：项目目标、常用命令、允许修改范围、禁止修改范围、公开内容边界、移动端验收、上线前检查、最终交付格式。\n不要写入密钥、账号、私密路径或内部研究对象。"
    },
    {
      title: "验收清单",
      cards: [
        ["命令真实", "命令来自项目文件，不是凭空猜测。"],
        ["边界明确", "哪些能改、哪些不能改、哪些要人工确认都写清楚。"],
        ["可复用", "下次 Codex 进入项目时能直接按这份规则工作。"]
      ]
    }
  ],
  "4-cli-terminal": [
    {
      title: "实战情景：用 CLI 接手远程项目",
      list: [
        "只有终端时，先让 Codex 建立命令地图。",
        "区分只读命令、写入命令、部署命令和危险命令。",
        "先跑低风险检查，再考虑 codex exec 自动化。",
        "所有涉及删除、发布、迁移、同步的命令都需要人工确认。"
      ]
    },
    {
      title: "CLI 接管提示词",
      code:
        "请在当前目录用只读方式检查项目。\n输出：项目用途、技术栈、包管理器、入口文件、安装/启动/构建/测试命令、可能写入或部署的命令、不能公开的文件、适合自动化的低风险任务。\n不要修改文件，不要读取 env 里的具体密钥值。"
    },
    {
      title: "codex exec 安全模板",
      code:
        "请只修改 docs/ 和 README 文件，把教程补成实战版本。\n禁止修改 src/、部署脚本、env、secrets、账号配置。\n完成后检查公开文档中是否出现密钥、内部来源、私密路径或不应公开的策略，并输出变更摘要。"
    }
  ],
  "5-cloud-github": [
    {
      title: "实战情景：Issue 到 PR",
      list: [
        "先把 Issue 写成可执行任务，而不是一句模糊愿望。",
        "写清楚背景、目标、范围、验收和禁止事项。",
        "让 Codex 先只读分析，再决定是否足够明确。",
        "PR 描述必须包含问题、改动、验证、风险和后续建议。"
      ]
    },
    {
      title: "Issue 模板",
      code:
        "标题：[一句话描述问题]\n背景：[用户看到什么]\n目标：[正确结果]\n范围：[允许修改]\n禁止修改：[密钥/部署/无关页面/账号配置]\n验收：[构建、测试、截图、路由、移动端]\n交付：PR + 验证结果 + 风险说明"
    },
    {
      title: "PR Review 提示词",
      code:
        "请以代码审查方式检查这个 PR。\n优先找问题：行为回归、安全和密钥风险、无关改动、移动端溢出或遮挡、缺失测试。\n先列 Findings，按严重程度排序；每条包含文件、行号、影响和建议修复；最后再给简短总结。"
    }
  ],
  "6-sandbox-permissions": [
    {
      ...
      list: [
        "只读扫描、总结文档、写计划属于低风险。",
        "修改指定源码、样式和文档属于中风险，需要范围约束。",
        "删除文件、部署、迁移、账号发布、密钥操作属于高风险，需要人工确认。",
        "所有高风险任务都要先写回滚路径。"
      ]
    },
    {
      title: "权限判断提示词",
      code:
        "在执行前，请先给这个任务做风险分级。\n请列出：需要读取的文件、可能修改的文件、要运行的命令、风险点、回滚方式、哪些步骤需要我确认。\n未经确认，不要执行删除、部署、账号发布、密钥或生产环境相关操作。"
    },
    {
      title: "验收清单",
      cards: [
        ["低风险", "只读分析、计划、总结，可以快速推进。"],
        ["中风险", "限定目录写入，必须有 diff 和验证。"],
        ["高风险", "先停下来说明影响、回滚和人工确认点。"]
      ]
    }
  ],
  "7-skills-mcp-plugins": [
    {
      title: "实战情景：把重复流程变成 Skill",
      list: [
        "先用普通对话跑通一次真实流程。",
        "记录触发场景、输入、步骤、禁止事项、输出和验收标准。",
        "把它写成 Skill，让 Codex 下次按同一套方法工作。",
        "涉及真实系统数据时，再接 MCP 或插件。"
      ]
    },
    {
      title: "Skill 草案提示词",
      code:
        "请把这个重复流程整理成一个 Skill 草案。\n请包含：适用场景、输入信息、执行步骤、安全边界、公开内容边界、输出格式、验收标准、何时必须停止等待人工确认。"
    },
    {
      title: "可做成 Skill 的任务",
      cards: [
        ["教程更新", "把一次实操改成章节、提示词、FAQ 和验收清单。"],
        ["前端验收", "固定检查 390px、768px、1440px 和截图证据。"],
        ["发布包制作", "把教程拆成公众号、小红书卡片、视频脚本和审核清单。"]
      ]
    }
  ],
  "8-agents-skills": [
    {
      title: "实战情景：沉淀内容生产 Skill",
      list: [
        "输入一次 Codex 实操记录。",
        "输出教程大纲、可复制提示词、卡片标题、视频脚本和 README 更新建议。",
        "保留人工审核清单，不做自动发布。",
        "公开内容不写内部研究对象、私密路径和敏感配置。"
      ]
    },
    {
      title: "内容 Skill 提示词",
      code:
        "请把这次 Codex 实操整理成内容资产。\n输出：中文教程大纲、可复制提示词、小红书 6 张卡片标题、60 秒短视频脚本、GitHub README 更新建议、人工审核清单。\n要求：不写内部研究对象，不暴露密钥、路径、账号和部署细节，文案自然引导读者找我学习。"
    },
    {
      title: "验收清单",
      cards: [
        ["能学习", "读者知道情景、步骤、提示词和验收方式。"],
        ["能发布", "拆出了卡片、脚本和 README 更新建议。"],
        ["能守边界", "没有泄露内部来源、密钥、账号和私密路径。"]
      ]
    }
  ],
  "9-real-workflows": [
    {
      title: "实战情景：读代码、做功能、修 Bug",
      list: [
        "读代码：只读扫描项目，不改文件，输出项目地图。",
        "做功能：先定位文件和最小方案，确认后再改。",
        "修 Bug：先复现或说明无法复现，再做最小修复。",
        "上线前：用构建、测试、截图或日志证明结果。"
      ]
    },
    {
      title: "功能实现提示词",
    ...
    },
    {
      title: "项目验收清单",
      cards: [
        ["读代码", "没有修改文件，输出技术栈、入口、命令和风险。"],
        ["做功能", "改动小、目标清楚、移动端可用、构建通过。"],
        ["修 Bug", "复现路径明确，有根因、有回归验证、有剩余风险。"]
      ]
    }
  ],
  "10-automation": [
    {
      ...
      list: [
        "先人工跑通流程，再考虑自动化。",
        "自动化只处理低风险、可验证、可回滚的任务。",
        "输出必须带证据：日志、截图、构建结果或报告。",
        "发布、账号、生产和付费动作保留人工确认。"
      ]
    },
    {
      title: "自动化设计提示词",
      code:
        "请把这个重复任务设计成自动化流程。\n请输出：触发条件、输入、允许修改、禁止修改、执行步骤、必须检查、输出证据、失败处理、人工确认点。\n不要直接执行发布、账号、密钥或生产环境操作。"
    },
    {
      title: "适合自动化的例子",
      cards: [
        ["文档更新", "检查链接、补 FAQ、生成章节摘要。"],
        ["教程拆分", "把实操记录拆成提示词、卡片和脚本。"],
        ["上线检查", "构建、路由、移动端、公开内容安全扫描。"]
      ]
    }
  ],
  "11-security-governance": [
    {
      title: "实战情景：公开前做敏感信息检查",
      list: [
        "检查公开页面、README、文档和提交记录。",
        "搜索密钥、token、cookie、SSH、OSS、DNS、env、私密路径。",
        "检查是否写入内部研究对象或不应公开的策略。",
        "发现问题先停止发布，修复后重新构建和验收。"
      ]
    },
    {
      title: "公开检查提示词",
      code:
        "请扫描公开文件，确认没有密钥、token、cookie、SSH、OSS、DNS、env、私密路径、内部研究对象或不应公开的策略。\n只汇报结果，不执行发布。\n请输出 PASS / WARN / BLOCKED、发现项、建议修复和需要人工确认的内容。"
    },
    {
      title: "治理验收清单",
      cards: [
        ["内容安全", "公开页面只写允许公开的信息。"],
        ["权限安全", "高风险命令和生产动作必须人工确认。"],
        ["回滚安全", "上线前知道如何恢复上一版。"]
      ]
    }
  ],
  "12-cheatsheets-faq": [
    {
      title: "实战情景速查",
      cards: [
        ["陌生仓库", "只读扫描项目用途、技术栈、入口、命令、风险和小任务。"],
        ["小功能", "目标、范围、禁止事项、验收标准先写清，再让 Codex 改。"],
        ["修 Bug", "现象、期望、复现路径、最小原因、回归验证缺一不可。"],
        ["PR Review", "先列 Findings，再给 Summary，重点看回归、安全、移动端和测试。"]
      ]
    },
    {
      title: "万能起手式",
      code:
        "请先只读定位相关文件，不要修改。\n输出：当前理解、相关文件、最小方案、风险点、验证方法、需要我确认的问题。\n等我确认后再执行。"
    },
    {
      title: "万能收尾式",
      code:
        "请总结本次任务：改了什么、为什么改、改了哪些文件、跑了哪些检查、结果如何、还有什么风险、下一步建议是什么。\n如果有未验证项，请明确列出，不要用笼统话带过。"
    }
  ]
};
...
function CodexTutorialDetail() {
  ...
  const sections = [...doc.sections, ...(tutorialScenarioSections[doc.slug] ?? [])];
